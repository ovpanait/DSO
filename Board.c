...
*/
 ...
 TIM4->DIER = (1 << UIE)           	//!<Update interrupt enable //
		| (0 << CC1IE)           		//!<Capture/Compare 1 interrupt enable //
		| (0 << CC2IE)           		//!<Capture/Compare 2 interrupt enable //
		| (0 << CC3IE)           		//!<Capture/Compare 3 interrupt enable //
		| (0 << CC4IE)           		//!<Capture/Compare 4 interrupt enable //
...
// NOTE: Remember to program ADC clock in RCC->CFGR
  ...
  ADC1->SR = (0 << AWD)                         	/*!<Analog watchdog flag */
		| (0 << EOC)                            /*!<End of conversion */
		| (0 << JEOC)                           /*!<Injected channel end of conversion */
		| (0 << JSTRT)                          /*!<Injected channel Start flag */
		| (1 << STRT);                          /*!<Regular channel Start flag */
  	
  ADC1->CR1 = (0 << AWDCH)            			/*!<AWDCH[4:0] bits (Analog watchdog channel select bits) */
			| (1 << EOCIE)           	/*!<Interrupt enable for EOC */
  ...
  ADC1->CR2 = (0 << ADON)        			/*!<A/D Converter ON / OFF */
			...
			| (1 << SWSTART)		/*!<Start Conversion of regular channels */
 ...
 //		110: 71.5 cycles
 //		111: 239.5 cycles
 ADC1->SMPR1 = (0 << SMP10) 	           	/*!<SMP10[2:0] bits (Channel 10 Sample time selection) */
 ...
 ADC1->SMPR2 = (0 << SMP0 )  	        	/*!<SMP0[2:0] bits (Channel 0 Sample time selection) */
 ...
 //
 ADC1->SQR1 = (0 << SQ13 )   			//            /*!<SQ13[4:0] bits (13th conversion in regular sequence) */
  ...
  ADC1->SQR2 = (0 << SQ7)     			//               /*!<SQ7[4:0] bits (7th conversion in regular sequence) */
  ...
  ADC1->SQR3 = (4 << SQ1)     			//             /*!<SQ1[4:0] bits (1st conversion in regular sequence) */
 ...
 //
  ADC1->JSQR = (0 << JSQ1)     			//            /*!<JSQ1[4:0] bits (1st conversion in injected sequence) */  
...
//  ADC2->DR;
 ...
 // Do calibration
 ADC1->CR2 |= (1 << CAL);     		
 while(!BitTest(ADC1->CR2, (1 << CAL))) {
 	// Wait for end of  calibration
 	}
 
 // Start ADC (the first ADON set turn on ADC power)
 ADC1->CR2 |= (1 << ADON);        	//           /*!<A/D Converter ON / OFF */
}
